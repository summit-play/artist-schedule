  ...
  return (
    <div className="flex flex-col bg-white text-black overflow-hidden text-left font-sans" style={{ ...dim }}>
      <div className={`flex items-center justify-between border-black ${!isExport ? 'mb-8 border-b-8 pb-4' : ''}`}
         ...
         <h2 className={`font-black uppercase italic leading-none ${!isExport ? 'text-4xl' : ''}`}
             style={isExport ? { fontSize: '280px' } : {}}>Scheduler</h2>
         <span className={`font-black uppercase ${!isExport ? 'text-lg' : ''}`}
               style={isExport ? { fontSize: '100px' } : {}}>틀어봐</span>
      </div>
      <div className={`flex items-center ${!isExport ? 'gap-6 mb-8' : ''}`}
           style={isExport ? { gap: '96px', marginBottom: '128px' } : {}}>
        <img src={order?.image} className={`object-cover shadow-xl border-slate-50 ${!isExport ? 'w-32 h-32 rounded-2xl border-4' : ''}`} 
             style={isExport ? { width: '700px', height: '700px', borderRadius: '6rem', borderWidth: '20px' } : {}} crossOrigin="anonymous" alt="" />
        <div className="space-y-2" style={isExport ? { display: 'flex', flexDirection: 'column', gap: '32px' } : {}}>
          <h3 className={`font-black uppercase leading-tight tracking-tight ${!isExport ? 'text-2xl' : ''}`}
              style={isExport ? { fontSize: '140px' } : {}}>{order?.artist}</h3>
          <p className={`font-bold text-slate-500 italic ${!isExport ? 'text-lg' : ''}`}
             style={isExport ? { fontSize: '90px' } : {}}>"{order?.title}"</p>
        </div>
      </div>
      <div className="flex-1" style={isExport ? { display: 'flex', flexDirection: 'column', gap: '48px', flex: 1 } : {}}>
        <div>
           <p className={`font-black text-slate-400 uppercase tracking-widest ${!isExport ? 'text-[10px] mb-1' : ''}`}
              style={isExport ? { fontSize: '48px', marginBottom: '24px' } : {}}>Product</p>
           <p className={`font-bold ${!isExport ? '' : ''}`}
              style={isExport ? { fontSize: '80px' } : {}}>{order?.productName}</p>
        </div>
        <div className={!isExport ? 'mt-8' : ''} style={isExport ? { marginTop: '64px' } : {}}>
           <p className={`font-black text-slate-400 uppercase tracking-widest ${!isExport ? 'text-[10px] mb-1' : ''}`}
              style={isExport ? { fontSize: '48px', marginBottom: '24px' } : {}}>Period</p>
           <p className={`font-black italic text-indigo-600 ${!isExport ? 'text-xl' : ''}`}
              style={isExport ? { fontSize: '120px' } : {}}>{order?.broadcastPeriod || 'TBA'}</p>
        </div>
        <div className={`bg-slate-50 border-dashed border-slate-200 shadow-inner ${!isExport ? 'mt-10 p-6 rounded-2xl border-2' : ''}`}
          ...
          <p className={`font-black text-slate-400 uppercase ${!isExport ? 'text-[10px] mb-2' : ''}`}
             style={isExport ? { fontSize: '48px', marginBottom: '40px' } : {}}>Description</p>
          <p className={`font-medium text-slate-600 italic break-words whitespace-pre-wrap ${!isExport ? 'text-sm' : ''}`}
             style={isExport ? { fontSize: '72px', lineHeight: '1.5' } : {}}>{desc || order?.description}</p>
        </div>
      </div>
      <div className={`mt-auto flex items-center justify-between opacity-20 border-slate-200 ${!isExport ? 'pt-4 border-t' : ''}`}
           style={isExport ? { marginTop: 'auto', paddingTop: '96px', borderTopWidth: '10px' } : {}}>
         <h4 className={`font-black tracking-[-0.2em] italic ${!isExport ? 'text-4xl' : ''}`}
             style={isExport ? { fontSize: '160px' } : {}}>틀어봐</h4>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="flex flex-col relative overflow-hidden font-sans" style={{ ...dim, backgroundColor: bgColor, color: 'white' }}>
      <div className={`text-center ${!isExport ? 'pt-24 px-10' : ''}`} style={isExport ? { paddingTop: '320px', paddingLeft: '80px', paddingRight: '80px' } : {}}>
        <div className={`relative aspect-square shadow-2xl mx-auto border-white/20 ${!isExport ? 'w-64 border-2' : ''}`}
             style={isExport ? { width: '800px', borderWidth: '20px', margin: '0 auto' } : {}}>
          <img src={order?.image} className="w-full h-full object-cover" crossOrigin="anonymous" alt="" />
        </div>
      </div>
      <div className={`flex flex-col ${!isExport ? 'px-10 mt-16 flex-1' : ''}`}
           style={isExport ? { paddingLeft: '96px', paddingRight: '96px', marginTop: '160px', flex: 1, display: 'flex', flexDirection: 'column' } : {}}>
        <div className="text-center" style={isExport ? { marginBottom: '64px' } : { marginBottom: '64px' }}>
          <h2 className={`font-black tracking-tighter uppercase break-keep leading-none ${!isExport ? 'text-4xl' : ''}`}
              style={isExport ? { fontSize: '120px', marginBottom: '40px' } : {}}>{order?.artist || '아티스트명'}</h2>
          <h4 className={`font-light opacity-80 italic ${!isExport ? 'text-xl mt-6' : ''}`}
              style={isExport ? { fontSize: '72px', marginTop: '24px' } : {}}>"{order?.title || '곡 제목'}"</h4>
        </div>
        <div className={`w-full bg-white/20 ${!isExport ? 'mb-10 h-px' : ''}`}
             style={isExport ? { height: '1px', marginBottom: '64px' } : {}}></div>
      ...
      <div className={`text-center font-black italic opacity-20 tracking-tighter ${!isExport ? 'pb-10 text-4xl mt-auto' : ''}`}
           style={isExport ? { paddingBottom: '128px', fontSize: '100px', marginTop: 'auto' } : {}}>틀어봐</div>
    </div>
  );
};
...
const App = () => {
  ...
  const extractColor = (imgUrl, orderId) => {
    if (!imgUrl) return;
    ...
    img.onload = () => {
      ...
      const finalColor = `rgb(${Math.floor(r/count)}, ${Math.floor(g/count)}, ${Math.floor(b/count)})`;
      setDominantColor(finalColor);
      if (orderId) updateOrderField(orderId, 'dominantColor', finalColor);
    };
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 1; i < rows.length; i++) {
        const values = rows[i]; if (!values.length) continue;
        const row = {}; headers.forEach((h, idx) => row[h] = values[idx]);
        const orderNo = String(row['주문번호'] || "").trim();
        if (!orderNo) continue;
        const optionStr = String(row['옵션명'] || "");
        const extract = (key) => { const m = optionStr.match(new RegExp(`${key}\\s*:\\s*([^/]+)`)); return m ? m[1].trim() : ""; };
        batch.set(doc(db, 'artifacts', appId, 'public', 'data', 'orders', orderNo), {
          orderNo, title: extract("곡 제목") || extract("노출 곡 제목") || "", artist: extract("아티스트명") || "",
          productName: row['상품명'] || "", description: optionStr, netPrice: row['품목실결제가'] || "0",
          orderDate: row['주문일'] || new Date().toISOString(), image: "https://placehold.co/400x400/000/fff?text=No+Image",
          status: '셋팅 필요', broadcastPeriod: '', updatedAt: new Date().toISOString()
        }, { merge: true });
        count++;
      }
      await batch.commit();
  ...
  if (publicViewId) {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center p-4 md:p-10 space-y-16 overflow-y-auto font-sans text-center">
        <header className="text-white flex flex-col items-center max-w-2xl text-center">
          ...
          <p className="text-sm opacity-60 mt-6 leading-relaxed italic">안녕하세요, <strong>{publicOrder.artist}</strong>님! 제작된 홍보 이미지를 확인해 주세요.<br/>하단 버튼을 눌러 고화질 원본 파일을 다운로드하실 수 있습니다.</p>
          <button onClick={() => downloadForArtist(publicOrder)} className="mt-10 px-12 py-5 bg-indigo-600 text-white rounded-full font-black text-sm flex items-center gap-3 shadow-2xl shadow-indigo-500/30 hover:scale-105 transition-transform"><Download size={20} /> 고화질 이미지 전체 다운로드</button>
        </header>
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      <aside className="w-72 bg-slate-900 text-white flex flex-col">
        <div className="p-10"><h1 className="text-3xl font-black italic uppercase">틀어봐</h1><p className="text-[10px] text-slate-500 mt-2 uppercase font-black tracking-widest">Admin Panel</p></div>
        <nav className="flex-1 px-6 space-y-2">
          <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-3xl transition-all ${activeTab === 'dashboard' ? 'bg-white text-black font-black shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}><LayoutDashboard size={20} /> <span className="text-sm font-bold text-left">대시보드</span></button>
          <button onClick={() => setActiveTab('orders')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-3xl transition-all ${activeTab === 'orders' ? 'bg-white text-black font-black shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}><ListOrdered size={20} /> <span className="text-sm font-bold text-left">주문 목록</span></button>
        </nav>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-2xl border-b px-10 py-6 flex items-center justify-between">
          <div className="relative w-full max-w-xl text-left"><Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300" size={20} /><input type="text" placeholder="검색..." className="w-full bg-slate-100 rounded-2xl py-3.5 pl-14 pr-8 text-sm font-bold outline-none focus:ring-2 focus:ring-black" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
          <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black uppercase shadow-lg">{user?.uid?.[0] || 'A'}</div>
        </header>

        <div className="p-10 space-y-10 text-left">
          {activeTab === 'dashboard' ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
              <div className="bg-white p-10 rounded-[3rem] border shadow-sm"><p className="text-[11px] font-black text-slate-400 uppercase mb-6">Total Campaigns</p><h3 className="text-6xl font-black">{orders.length}</h3></div>
              <div className="bg-white p-10 rounded-[3rem] border shadow-sm"><p className="text-[11px] font-black text-emerald-500 uppercase mb-6 text-left">Live</p><h3 className="text-6xl font-black text-emerald-600">{orders.filter(o => o.status === '송출중').length}</h3></div>
              <div className="bg-white p-10 rounded-[3rem] border shadow-sm"><p className="text-[11px] font-black text-amber-500 uppercase mb-6 text-left">Waiting</p><h3 className="text-6xl font-black text-amber-600">{orders.filter(o => o.status === '셋팅 필요').length}</h3></div>
            </div>
          ) : (
            <div className="space-y-8">
              <div className="flex gap-3 overflow-x-auto pb-2 text-left">{['전체', ...CUSTOM_STATUSES].map(s => <button key={s} onClick={() => setFilterStatus(s)} className={`px-8 py-3 rounded-full text-xs font-black border whitespace-nowrap ${filterStatus === s ? 'bg-slate-900 text-white shadow-xl' : 'bg-white text-slate-500 hover:border-slate-400'}`}>{s}</button>)}</div>
              <div className="bg-white rounded-[3.5rem] border shadow-sm overflow-hidden text-sm">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-widest text-left"><tr><th className="px-8 py-6 text-center">No</th><th className="px-8 py-6">Artist / Track</th><th className="px-8 py-6 text-right">Price</th><th className="px-8 py-6 text-center">Period</th><th className="px-8 py-6">Status</th><th className="px-8 py-6 text-center">Studio</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {filteredOrders.map(o => (
                      <tr key={o.id} className="hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => setSelectedOrderIdForDetail(o.id)}>
                        ...
                        <td className="px-8 py-7 flex items-center gap-4"><img src={o.image} className="w-14 h-14 rounded-xl object-cover shadow-sm group-hover:scale-110 transition-transform" /><div className="flex flex-col"><h4 className="font-black leading-tight text-slate-900">{o.title}</h4><p className="text-xs text-slate-400 font-bold uppercase">{o.artist}</p></div></td>
                        <td className="px-8 py-7 text-right font-black text-slate-700">{Number(o.netPrice).toLocaleString()}원</td>
                        <td className="px-8 py-7 text-center" onClick={e => e.stopPropagation()}><input type="text" className="bg-slate-100/50 border-none rounded-lg py-1.5 px-3 text-[11px] font-bold w-32 text-center outline-none focus:ring-1 focus:ring-black" value={o.broadcastPeriod || ''} onChange={e => updateOrderField(o.id, 'broadcastPeriod', e.target.value)} placeholder="00.00-00.00" /></td>
                        ...
                        <td className="px-8 py-7 text-center" onClick={e => e.stopPropagation()}><button onClick={() => openStudio(o)} className="p-3 bg-slate-100 text-slate-900 rounded-xl hover:bg-black hover:text-white transition-all shadow-sm"><Send size={16}/></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selectedOrderIdForDetail && selectedOrderForDetail && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-[4rem] w-full max-w-5xl h-full max-h-[90vh] flex flex-col overflow-hidden text-left shadow-2xl animate-in fade-in zoom-in-95 duration-300">
            <header className="p-10 border-b flex items-center justify-between"><h2 className="text-2xl font-black italic uppercase text-slate-900 tracking-tighter">Details</h2><button onClick={() => setSelectedOrderIdForDetail(null)} className="p-4 hover:bg-slate-100 rounded-full transition-colors text-slate-400"><X size={24} /></button></header>
            <div className="flex-1 overflow-y-auto p-12 grid grid-cols-1 md:grid-cols-3 gap-12 text-left">
              <div className="space-y-6 text-center">
                <div className="relative group overflow-hidden rounded-[3rem] shadow-2xl border border-slate-100"><img src={selectedOrderForDetail.image} className="w-full aspect-square object-cover" /><label className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"><Camera size={32} className="text-white" /><input type="file" accept="image/*" className="hidden" onChange={e => handleImageUpload(selectedOrderForDetail.id, e.target.files[0])} /></label></div>
                <div className="p-6 bg-slate-50 border rounded-[2.5rem] shadow-inner text-center"><p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest italic">Schedule</p><input type="text" className="w-full bg-white border rounded-2xl py-3 px-4 text-center font-black text-sm focus:ring-2 focus:ring-indigo-500 outline-none" value={selectedOrderForDetail.broadcastPeriod || ''} onChange={e => updateOrderField(selectedOrderForDetail.id, 'broadcastPeriod', e.target.value)} placeholder="MM.DD-MM.DD" /></div>
              </div>
              <div className="md:col-span-2 space-y-10">
                <div className="flex gap-10 border-b pb-10">
                  <div className="flex-1 text-left"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Artist</p><p className="font-black text-3xl text-slate-900 tracking-tight">{selectedOrderForDetail.artist}</p></div>
                  <div className="flex-1 text-left"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Track</p><p className="font-black text-3xl text-slate-900 opacity-70 italic tracking-tight">"{selectedOrderForDetail.title}"</p></div>
                </div>
                <div className="space-y-4 text-left">
                  <p className="text-[10px] font-black text-slate-400 uppercase italic tracking-widest text-left">Album Description</p>
            ...
            <footer className="p-10 border-t flex gap-6 bg-slate-50">
              <button onClick={() => { openStudio(selectedOrderForDetail); setSelectedOrderIdForDetail(null); }} className="px-12 py-5 bg-white border-2 rounded-[2rem] font-black text-sm hover:bg-black hover:text-white transition-all shadow-lg flex items-center gap-2 tracking-tighter"><FileImage size={18} /> 스튜디오</button>
              <button onClick={() => copySmsText(selectedOrderForDetail)} className="flex-1 py-5 bg-black text-white rounded-[2rem] font-black text-sm hover:scale-[1.02] transition-all shadow-2xl shadow-black/30 flex items-center justify-center gap-2 text-white text-white"><MessageSquareShare size={20} /> 문자 복사</button>
            </footer>
          </div>
        </div>
      )}
      ...
      {selectedOrderIdForMessage && selectedOrderForMessage && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-8 bg-black/98 backdrop-blur-3xl animate-in zoom-in-95 duration-300">
          <div className="bg-white rounded-[4rem] shadow-2xl w-full max-w-7xl h-full flex overflow-hidden text-left">
            <div className="w-[420px] border-r p-12 flex flex-col bg-slate-50">
              <div className="flex items-center justify-between mb-12"><h2 className="text-2xl font-black italic uppercase text-slate-900 tracking-tighter">Studio</h2><button onClick={() => { setSelectedOrderIdForMessage(null); setMessagePage(1); }} className="p-3 hover:bg-slate-200 rounded-full transition-colors text-slate-400"><X size={24} /></button></div>
              <div className="flex-1 space-y-6 overflow-y-auto scrollbar-hide text-left">
                <div className="flex gap-2 text-center">{[1, 2, 3].map(p => <button key={p} onClick={() => setMessagePage(p)} className={`flex-1 py-3 rounded-2xl text-[11px] font-black transition-all ${messagePage === p ? 'bg-black text-white shadow-lg' : 'bg-white border text-slate-400'}`}>{p === 1 ? '티저' : p === 2 ? '스케줄' : '포스터'}</button>)}</div>
                <div className="space-y-4">
                  <div className="text-[10px] font-black text-slate-400 uppercase px-2 italic tracking-widest text-left">Preview Description</div>
                  <div className="w-full h-40 p-6 bg-white border rounded-[2.5rem] text-xs font-bold leading-relaxed overflow-y-auto italic text-slate-800 shadow-inner whitespace-pre-wrap">{selectedOrderForMessage.description}</div>
                </div>
              </div>
                ...
                <button 
                  onClick={() => {
                    window.html2canvas(posterRef.current, { useCORS: true, scale: 2 }).then(canvas => {
                      ...
                      a.download = `틀어봐_${selectedOrderForMessage.artist}_P${messagePage}.png`;
            ...
            <div className="flex-1 bg-[#0a0a0a] p-12 overflow-y-auto flex justify-center items-center">
              <div ref={posterRef} className="relative shadow-2xl overflow-hidden flex flex-col transition-all duration-300" style={{ width: messagePage === 3 ? '353px' : (messagePage === 2 ? '450px' : '400px'), minHeight: messagePage === 3 ? '857px' : (messagePage === 2 ? '500px' : '711px'), backgroundColor: messagePage === 2 ? 'white' : (selectedOrderForMessage.dominantColor || dominantColor), color: messagePage === 2 ? 'black' : 'white', borderRadius: messagePage === 3 ? '0px' : '10px' }}>
