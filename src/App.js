    ...
    <div className=“flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden”>
      <aside className=“w-72 bg-slate-900 text-white flex flex-col”>
        ...
        <nav className=“flex-1 px-6 space-y-2”>
          <button onClick={() => setActiveTab(‘dashboard’)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-3xl transition-all ${activeTab === ‘dashboard’ ? ‘bg-white text-black font-black’ : ‘text-slate-400’}`}><LayoutDashboard size={20} /> <span className=“text-sm font-bold text-left”>대시보드</span></button>
          <button onClick={() => setActiveTab(‘orders’)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-3xl transition-all ${activeTab === ‘orders’ ? ‘bg-white text-black font-black’ : ‘text-slate-400’}`}><ListOrdered size={20} /> <span className=“text-sm font-bold text-left”>주문 목록</span></button>
        </nav>
        ...
        <div className=“fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/60 backdrop-blur-sm”>
          <div className=“bg-white rounded-[4rem] w-full max-w-5xl h-full max-h-[90vh] flex flex-col overflow-hidden text-left shadow-2xl animate-in fade-in zoom-in-95 duration-300">
            ...
            <footer className=“p-10 border-t flex gap-6 bg-slate-50">
              ...
              <button onClick={() => copySmsText(selectedOrderForDetail)} className=“flex-1 py-5 bg-black text-white rounded-[2rem] font-black text-sm hover:scale-[1.02] transition-all shadow-2xl shadow-black/30 flex items-center justify-center gap-2"><MessageSquareShare size={20} /> 문자 복사</button>
            </footer>
          </div>
        </div>
      )}
            ...
            <div className=“w-[420px] border-r p-12 flex flex-col bg-slate-50”>
              ...
              <div className=“flex-1 space-y-10 overflow-y-auto scrollbar-hide text-left”>
                ...
                <div className=“space-y-4”>
                  ...
                  <div className=“w-full h-64 p-8 bg-white border rounded-[2.5rem] text-xs font-bold leading-relaxed overflow-y-auto italic text-slate-800 shadow-inner whitespace-pre-wrap”>{selectedOrderForMessage.description}</div>
                </div>
              </div>
